...
import org.jubaroo.mods.wyvernitems.items.AffinityOrb;
...
public class AffinityOrbAction implements ModAction {
	...
	public BehaviourProvider getBehaviourProvider()
	{
		return new BehaviourProvider() {
			...
			{
				if(performer instanceof Player && object != null && object.getTemplateId() == AffinityOrb.getId()) {
		...
		return new ActionPerformer() {
			...
			public boolean action(Action act, Creature performer, Item target, short action, float counter)
			{
				if(performer instanceof Player){
					...
					if (target.getTemplate().getTemplateId() != AffinityOrb.getId()){

...
import org.jubaroo.mods.wyvernitems.items.ArrowPackHunting;
import org.jubaroo.mods.wyvernitems.items.ArrowPackWar;
...
public class ArrowPackUnpackAction implements ModAction {
	...
	public BehaviourProvider getBehaviourProvider()
	{
		return new BehaviourProvider() {
			...
			{
				if(performer instanceof Player && object != null && (object.getTemplateId() == ArrowPackHunting.getId() || object.getTemplateId() == ArrowPackWar.getId())) {
			...
			public boolean action(Action act, Creature performer, Item target, short action, float counter)
			{
				try{
					if(performer instanceof Player){
						if(target.getTemplate().getTemplateId() != ArrowPackHunting.getId() && target.getTemplate().getTemplateId() != ArrowPackWar.getId()){
						...
						if(counter == 1.0f){
							...
							if(target.getTemplate().getTemplateId() == ArrowPackWar.getId()){

...
public class EnchantOrbAction implements ModAction {
	...
	public BehaviourProvider getBehaviourProvider()
	{
		return new BehaviourProvider() {
			...
			{
				if(performer instanceof Player && source != null && object != null && source.getTemplateId() == EnchantOrb.getId() && source != object){
		...
		return new ActionPerformer() {
			...
			public boolean action(Action act, Creature performer, Item source, Item target, short action, float counter)
			{
				if(performer instanceof Player){
					...
					if(source.getTemplate().getTemplateId() != EnchantOrb.getId()){
						player.getCommunicator().sendNormalServerMessage("You must use an Enchant Orb to transfer enchants.");
						return true;
					}
					if(target.getTemplate().getTemplateId() == EnchantOrb.getId()){

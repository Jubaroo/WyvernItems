...
public class AffinityCatcherConsumeAction implements ModAction {
	...
	public BehaviourProvider getBehaviourProvider()
	{
		return new BehaviourProvider() {
			...
			{
				if(performer instanceof Player && object != null && object.getTemplateId() == AffinityCatcher.getId() && object.getData() > 0) {
		...
		return new ActionPerformer() {
			...
			public boolean action(Action act, Creature performer, Item target, short action, float counter)
			{
				if(performer instanceof Player){
					...
					if (target.getTemplate().getTemplateId() != AffinityCatcher.getId()){

...
import org.jubaroo.mods.wyvernitems.items.EnchantOrb;
import org.jubaroo.mods.wyvernitems.items.EternalOrb;
...
public class EternalOrbAction implements ModAction {
	...
	public BehaviourProvider getBehaviourProvider()
	{
		return new BehaviourProvider() {
			...
			{
				if(performer instanceof Player && source != null && object != null && source.getTemplateId() == EternalOrb.getId() && source != object){
		...
		return new ActionPerformer() {
			...
			public boolean action(Action act, Creature performer, Item source, Item target, short action, float counter)
			{
				if(performer instanceof Player){
					...
					if(source.getTemplate().getTemplateId() != EternalOrb.getId()){
					...
					try {
						Item enchantOrb = ItemFactory.createItem(EnchantOrb.getId(), source.getCurrentQualityLevel(), "");
